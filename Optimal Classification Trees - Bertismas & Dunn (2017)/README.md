...
| :--- | :--- | :--- | :--- |
| **Node Indexing** | `t ∈ T_B = {1, ..., ⌊T/2⌋}`<br>`t ∈ T_L = {⌊T/2⌋ + 1, ..., T}` | `branches = list(range(num_branches))` <br> `leaves = list(range(...))` | The paper uses **1-based indexing** (1 to T), while the Python code uses standard **0-based indexing** (0 to 2^(D+1) - 2). |
| **Feature Selection** | `∑ a_jt = d_t` for all `t ∈ T_B`<br>*(Eq. 2)* | `d = model.addVars(branches, P, vtype=GRB.BINARY)`<br>`gp.quicksum(d[t, j] for j in range(P)) <= 1` | The paper explicitly separates feature choice `a_jt ∈ {0, 1}` and split flag `d_t ∈ {0, 1}`. The code collapses both into a single 2D binary variable `d[t, j]`. |
| **Parent Hierarchy** | `d_t ≤ d_p(t)` for all `t ∈ T_B \ {1}`<br>*(Eq. 5)* | `gp.quicksum(d[t, j]) <= gp.quicksum(d[parent, j])` | **Exact Alignment:** Enforces that a child node cannot execute a split unless its parent node has already split. |
| **Leaf Allocation** | `∑ z_it = 1` for all `i = 1, ..., n` across `t ∈ T_L`<br>*(Eq. 8)* | `gp.quicksum(z[i, l] for l in leaves) == 1` | **Exact Alignment:** Guarantees every observation `i` is routed to exactly one leaf node. |
| **Left Ancestor Routing** | `a_m^T(x_i + ε) ≤ b_m + (1 + ε_max)(1 - z_it)`<br>*(Eq. 13)* | `gp.quicksum(d[t, j] * X[i, j]) <= b[t] + M * (1 - z[i, l])` | The paper derives feature-specific tolerances `ε_j` and tight Big-M bounds (`1 + ε_max`). The code applies a dynamic scalar `M = max(|X|) + 5.0`. |
| **Right Ancestor Routing** | `a_m^T x_i ≥ b_m - (1 - z_it)`<br>*(Eq. 14)* | `gp.quicksum(d[t, j] * X[i, j]) >= b[t] - M * (1 - z[i, l])` | The paper assumes normalized features `x_i ∈ [0, 1]^P` (setting M = 1). The code uses dynamic M scaling to handle unnormalized feature values. |
| **Misclassification Loss** | `L_t = N_t - max_k(N_kt)`<br>*(Eq. 19–22)* | `e[i, l] >= z[i, l] - c[l]` (if `y_i = 1`)<br>`e[i, l] >= c[l] + z[i, l] - 1` (if `y_i = 0`) | The paper computes class distribution counts `N_kt` per leaf. The code uses sample-level binary error variables `e_il` tailored for binary classification (K = 2). |
| **Objective Function** | `min (1/L̂) ∑ L_t + α ∑ d_t`<br>*(Eq. 24)* | `model.setObjective(misclass_error + tree_complexity, GRB.MINIMIZE)` | The paper scales classification loss by baseline error `L̂`. The code minimizes raw total misclassifications plus the complexity penalty `α ∑ d_tj`. |
| **Minimum Leaf Size** | `∑ z_it ≥ N_min * l_t` for all `t ∈ T_L`<br>*(Eq. 7)* | *Omitted in current Python code* | **Paper Constraint Omitted:** The paper enforces a minimum sample count threshold `N_min` per active leaf node using binary leaf indicators `l_t`. |